...
# handles sb, sh, sw — immediate is split across two fields
def decode_s(instr):
    funct3  = get_bits(instr, 14, 12)
    rs1     = get_bits(instr, 19, 15)
    rs2     = get_bits(instr, 24, 20)
...
# handles beq, bne, blt, bge — immediate bits are scrambled all over the place
def decode_sb(instr):
    funct3  = get_bits(instr, 14, 12)
    rs1     = get_bits(instr, 19, 15)
    rs2     = get_bits(instr, 24, 20)
...
# only handles jal — immediate bits are even more scrambled than SB
def decode_uj(instr):
    rd = get_bits(instr, 11, 7)
...
# opcode table — maps the lowest 7 bits to the right decoder function
DECODERS = {
    51: decode_r,
    19: decode_i,
    3: decode_i,
...
def main():
    while True:
        ...
        if len(bits) != 32:
            print("Error: expected a 32-bit binary string.\n")
            continue
